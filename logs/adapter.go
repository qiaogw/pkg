...
func (zapAdapter *zapAdapter) Build(mode string) {
	fileWriter := zapcore.AddSync(zapAdapter.createLumberjackHook())
	...
	if mode == "dev" {
		allCore = append(allCore, zapcore.NewCore(cnf, consoleOut, lowPriority))
		allCore = append(allCore, zapcore.NewCore(cnf, fileWriter, lowPriority))
	} else {
		allCore = append(allCore, zapcore.NewCore(cnf, consoleOut, highPriority))
		allCore = append(allCore, zapcore.NewCore(cnf, fileWriter, highPriority))
	}
