...
// Init init logger
func Init(mode, path, level string, needRequestLog bool, options ...LogOption) {
	logger = &Log{Path: path, Level: level}
	logger.NeedRequestLog = needRequestLog
	logger.createFiles(mode, level, needRequestLog, options...)
}
...
func (l *Log) createFiles(mode, level string, needRequestLog bool, options ...LogOption) {
