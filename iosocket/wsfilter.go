...
import (
	...
	"github.com/qiaogw/pkg/iosocket/sessionmanager"
...
func New() *Notifier {
	...
	m.HandleConnect(func(s *melody.Session) {
		...
		uid, exists := s.Get("userid")
		if !exists {
			return
		}
		rid, exists := s.Get("roomid")
		if !exists {
			return
		}
		roomId := rid.(string)
		...
		userlist := GetUserlist(Subscribers, roomId)
		evt := NewEvent(models.EVENT_JOIN, user.Abbr, user.Id, user.Photo, roomId, "新用户连接", userlist)
		sub := Subscriber{
			Id:     user.Id,
			Name:   user.Abbr,
			Avatar: user.Photo,
...
func (n *Notifier) GroupBroadcast(msg []byte, groupID string) error {
	beego.Debug("groupID :", string(msg), groupID, n.SessionManager.ShowSessions())
	if n.GroupLen(groupID) == 0 {
		return errors.New("no active session")
	}
	return n.Melody.BroadcastFilter(msg, func(s *melody.Session) bool {
		group, exists := s.Get("group")
		beego.Debug("group, exists :", msg, group, exists, groupID)
